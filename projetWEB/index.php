...
if(isset($_POST['formConnexion'])){
  ...
  if(!empty($loginConnect) AND !empty($motDePasseConnect)){
    $sql =("SELECT * FROM id12822867_projetweb.UTILISATEUR WHERE UTILISATEUR.login = ? AND UTILISATEUR.motDePasse = ?");
    ...
    if($utilisateurExist == 1){
      ...
      header("Location: accueil/login/".$utilisateurInfo['login']);
